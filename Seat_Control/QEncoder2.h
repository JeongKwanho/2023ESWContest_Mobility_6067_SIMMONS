...
class QEncoder{
    private:
        ...
        const float alpha;
        const uint32_t dt;
        float las_rpm,las_rate, fil_rpm, fil_rate;
        uint16_t CPR;
        float gear_ratio;
        const char dir;
    ...
    float CalRPM(){
        float enc_rpm = (float)pulse_count*60*1000/(dt*CPR*gear_ratio*4*4);
    ...
    float CalRATE(){
        ///RATE = RPM*
        float enc_rate = (float)pulse_count*1000*2*_PI/(dt*CPR*gear_ratio*4*4);//각속도
        fil_rate = LPF(&las_rate,enc_rate);
        pulse_count = 0;
        return fil_rate;
    }
